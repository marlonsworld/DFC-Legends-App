...
(() => {
  ...
  // GIF is accepted and passed through byte-for-byte (never canvas-encoded)
  // so animation survives; everything else goes through the downscale.
  ...
  async function toDataUrl(file, targetW) {
    if (PASSTHROUGH.indexOf(file.type) >= 0) {
      if (file.size > MAX_PASSTHROUGH_BYTES) throw new Error('GIF too large (max 12MB).');
